...
import com.vw.commentengine.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link com.vw.commentengine.domain.CommentTarget}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CommentTargetResource {
    ...
    /**
     * {@code POST  /comment-targets} : Create a new commentTarget.
     *
     * @param commentTarget the commentTarget to create.
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new commentTarget, or with status {@code 400 (Bad Request)} if the commentTarget has already an ID.
    ...
    public ResponseEntity<CommentTarget> createCommentTarget(@RequestBody CommentTarget commentTarget) throws URISyntaxException {
        ...
        if (commentTarget.getId() != null) {
            throw new BadRequestAlertException("A new commentTarget cannot already have an ID", ENTITY_NAME, "idexists");
        }
    ...
    /**
     * {@code PUT  /comment-targets} : Updates an existing commentTarget.
     *
     * @param commentTarget the commentTarget to update.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated commentTarget,
     * or with status {@code 400 (Bad Request)} if the commentTarget is not valid,
     * or with status {@code 500 (Internal Server Error)} if the commentTarget couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<CommentTarget> updateCommentTarget(@RequestBody CommentTarget commentTarget) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, commentTarget.getId().toString()))
            .body(result);
    }

    /**
     * {@code GET  /comment-targets} : get all the commentTargets.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of commentTargets in body.
     */
    ...
    /**
     * {@code GET  /comment-targets/:id} : get the "id" commentTarget.
     *
     * @param id the id of the commentTarget to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the commentTarget, or with status {@code 404 (Not Found)}.
     */
    ...
    public ResponseEntity<CommentTarget> getCommentTarget(@PathVariable Long id) {
        ...
        Optional<CommentTarget> commentTarget = commentTargetRepository.findById(id);
        return ResponseUtil.wrapOrNotFound(commentTarget);
    }

    /**
     * {@code DELETE  /comment-targets/:id} : delete the "id" commentTarget.
     *
    ...
    public ResponseEntity<Void> deleteCommentTarget(@PathVariable Long id) {
        ...
        commentTargetRepository.deleteById(id);
